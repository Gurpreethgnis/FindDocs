...
function App() {
  ...
  const getCurrentConversation = () => {
    const conversation = chatHistory.find(conv => conv.id === currentConversationId);
    console.log('getCurrentConversation called:', {
      currentConversationId,
      chatHistoryLength: chatHistory.length,
      foundConversation: conversation,
      allConversationIds: chatHistory.map(c => c.id)
    });
    return conversation;
  };
  ...
  const processSingleFile = async (file) => {
    ...
    try {
      ...
      formData.append('files', file);
              ...
              console.log('Sending file to Gradio API...');
        
        // Use the simple Gradio file upload endpoint
        const response = await axios.post(`${DOCLING_URL}/ui/gradio_api/upload`, formData, {
          headers: {
      ...
      if (response.data) {
        console.log('Document object keys:', Object.keys(response.data));
        console.log('Document content types available:', {
          ...
          json_content: !!response.data.json_content
        });
        
        if (response.data.md_content) {
          console.log('Markdown content preview:', response.data.md_content.substring(0, 500));
        }
        if (response.data.text_content) {
          console.log('Text content preview:', response.data.text_content.substring(0, 500));
        }
        
        // Log the full document object for debugging
        console.log('Full document object:', JSON.stringify(response.data, null, 2));
      }
      
      // Extract content from response
      const content = response.data?.md_content || 
      ...
      if (response.data) {
        ...
        console.log('Document object:', response.data);
        console.log('MD content length:', response.data?.md_content?.length || 0);
        console.log('Text content length:', response.data?.text_content?.length || 0);
      ...
      console.error('File being processed:', file.name);
      ...
      console.error('Docling URL:', DOCLING_URL);
      console.error('Request method: POST');
      console.error('Request endpoint: /v1/convert');
      console.error('FormData contents:');
      for (let [key, value] of formData.entries()) {
        if (key === 'options') {
          console.error(`${key}: [JSON Blob]`);
        } else {
  ...
  const processEntireDirectory = async () => {
    ...
    for (let i = 0; i < newFiles.length; i++) {
      ...
      try {
        ...
        console.log('Gradio response received:', response.data);
        
        // Extract content from response (adjust based on Gradio API response format)
        ...
        if (response.data) {
          ...
          console.log('=== BATCH DOCUMENT PROCESSING DEBUG ===');
          console.log('File processed:', newDoc.filename);
          console.log('Content length:', newDoc.content.length);
          console.log('=== END DEBUG ===');
        ...
        // Update progress
        const progressPercentage = Math.round(((i + 1) / newFiles.length) * 100);
        setUploadStatus(`Processed ${file.name} (${i + 1}/${newFiles.length}) - ${progressPercentage}% complete`);

      } catch (error) {
        console.error(`Error processing ${file.name}:`, error);
        setUploadStatus(`Error processing ${file.name}: ${error.message}`);
        failedCount++;
    ...
    // Final status update
    const successMessage = `Batch processing complete! ${processedCount} files processed successfully${failedCount > 0 ? `, ${failedCount} failed` : ''}`;
    setUploadStatus(successMessage);
    setIsScanningDirectory(false);
    setScanProgress({ current: 0, total: 0, skipped: 0 });
...
Answer:`;
      ...
      if (!currentConversationId) {
        ...
        // Wait for the next render cycle, then add the messages
        setTimeout(() => {
          setChatHistory(prev => {
            const lastConv = prev[prev.length - 1];
            if (lastConv) {
              console.log('Adding messages to new conversation:', lastConv.id);
              ...
              console.log('Updated chat history after new conversation:', updated);
              return updated;
            }
            return prev;
          });
  ...
  useEffect(() => {
    if (!currentConversationId && chatHistory.length === 0) {
      console.log('Auto-starting conversation from useEffect');
      startNewConversation();
    }
  ...
  const openDocument = (document) => {
    try {
      ...
      if (document.originalFile && document.originalFile instanceof File) {
        ...
        try {
          ...
          console.error('Error creating object URL:', fileError);
          // Fall through to next method
        }
      } else {
        console.log('Original file is not a valid File object:', document.originalFile);
      }
